...
Данный скрипт автоматизации упрощает развертывание обратного прокси-сервера с использованием NGINX и XRAY, а также установку панели управления и ноды Remnawave. Архитектура оптимизирована для производительности: Xray работает непосредственно на порту 443, терминирует TLS для каждого публичного хостнейма и держит цепочку сертификатов плюс статическую пару ECH-ключей. Веб-сервер за Xray (NGINX или Caddy) слушает Unix-сокет в cleartext, минимизируя накладные расходы TCP и улучшая надежность соединений. Единый Xray-инбаунд обслуживает VLESS, Trojan и Shadowsocks через WebSocket, HTTPUpgrade, XHTTP и TCP+HTTP-obfs транспорты на одном порту, маршрутизация по path-фолбэкам.

[!IMPORTANT]
Поддержка Debian и Ubuntu. Cкрипт был протестирован в среде виртуализации KVM. Для корректной работы вам потребуются собственные домены. Рекомендуется запускать с правами root на свежеустановленной системе.
...
· Сервер ноды: Размещает XRAY ноду и сайт-заглушку на прямом домене

Требования к доменам

Подготовьте два домена или поддомена перед установкой:

1. CDN домен: Панель по / и страница подписки по /sub
2. Прямой домен: Хостнейм прокси, сайт-заглушка и ECH serverName

Отдельный домен для подписки опционален и включается ответом y на вопрос о раздельной схеме при установке.
...
Также поддерживаются DNS API Gcore и Bunny.net для выпуска wildcard-сертификатов.
...
[!TIP]
CDN домен может стоять за Cloudflare (proxied). Прямой домен должен оставаться DNS only — Xray терминирует на нём TLS, а ECH требует прямого хендшейка.
...
4. Введите CDN домен, ответьте на вопрос о раздельной подписке (по умолчанию N), введите прямой домен
5. Дождитесь завершения
6. Скрипт автоматически перезапустит сервисы, отобразит учётные данные и напечатает ECHConfigList для публикации
...
4. Сохраните предоставленные учётные данные и напечатанный ECHConfigList
...
4. Введите прямой домен и вставьте сертификат при запросе
...
· Единый Xray-инбаунд на 443 с VLESS, Trojan и Shadowsocks через WebSocket, HTTPUpgrade, XHTTP и TCP+HTTP-obfs
· TLS терминируется Xray, статический ECH включён
